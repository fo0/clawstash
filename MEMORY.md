...
- **Mermaid render concurrency + cold-import timing (#286, 2026-06-14)** — two latent bugs made inline ` ```mermaid ` diagrams render blank on full page load / F5 (but fine via SPA nav, where the chunk is warm and the app is idle). (1) Mermaid keeps mutable global state during `render()`, so the old parallel `forEach` hydration (and side-by-side table layouts) issued overlapping `render()` calls that clobbered each other → blank SVGs. Fixed by serializing all `renderMermaid` calls through a module-level promise chain (`renderChain` in `src/utils/mermaid.ts`). (2) The hydration effect captured the placeholder NodeList up front; on a cold load the lazy `import('mermaid')` resolved after a re-render had replaced those nodes, and the late `.then()` silently dropped the now-detached node with no retry. Fixed by making the pass an async sequential loop that re-queries the live DOM each turn. Don't revert either to a parallel/capture-once pattern.
