...
function computeRadius(node: StashGraphNode): number {
  if (node.type === 'stash')
    return Math.max(10, Math.min(28, 10 + Math.sqrt(node.file_count || 1) * 4));
  if (node.type === 'tag') return Math.max(6, Math.min(20, 6 + Math.sqrt(node.count || 1) * 3));
  return 5; // version
}

function buildRenderNodes(data: StashGraphResult): {
  nodes: RenderNode[];
...
function simulate(nodes: RenderNode[], edges: StashGraphEdge[], alpha: number) {
  ...
  for (const edge of edges) {
    ...
    const strength =
      edge.type === 'has_tag'
        ? 0.012
        : edge.type === 'shared_tags'
          ? 0.01
          : edge.type === 'version_of'
            ? 0.02
            : 0.002;
...
export default function StashGraphCanvas({
  onSelectStash,
  analyzeStashId,
  ...
  const applyVisibilityFilter = useCallback(
    (
      ...
      let activeEdges = allEdges.filter(
        (e) => !ignoredTagIds.has(e.source) && !ignoredTagIds.has(e.target),
      );
      ...
      if (analysed.size === 0) {
        ...
        const visibleEdges = activeEdges.filter(
          (e) => e.type === 'shared_tags' && visibleIds.has(e.source) && visibleIds.has(e.target),
        );
      ...
      const tagBasedEdges = activeEdges.filter(
        (e) => e.type === 'has_tag' || e.type === 'shared_tags',
      );
      ...
      const visibleActiveEdges = activeEdges.filter((e) => {
        ...
        if (analysed.size > 0 && e.type === 'has_tag') {
          const stashEnd =
            nodeTypeMap.get(e.source) === 'stash'
              ? e.source
              : nodeTypeMap.get(e.target) === 'stash'
                ? e.target
                : null;
      ...
      const visibleIgnoredEdges = ignoredTagEdges.filter(
  ...
  useEffect(() => {
    if (analyzeStashId && allNodesRef.current.length > 0) {
      const stashNode = allNodesRef.current.find(
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((data) => {
        ...
        const { nodes: filtered, edges: filteredEdges } = applyVisibilityFilter(
          nodes,
          edges,
          initialAnalysed,
  ...
  useEffect(() => {
    ...
    const { nodes, edges } = applyVisibilityFilter(
      allNodesRef.current,
      allEdgesRef.current,
      analysedStashes,
      ignoredTags,
  ...
  const screenToWorld = useCallback((sx: number, sy: number, canvas: HTMLCanvasElement) => {
    const cx = canvas.width / (2 * devicePixelRatio),
      cy = canvas.height / (2 * devicePixelRatio);
    return {
  ...
  const findNodeAt = useCallback((wx: number, wy: number) => {
    for (let i = nodesRef.current.length - 1; i >= 0; i--) {
      ...
      const hitR =
        node.type === 'stash' ? Math.max(node.radius * 1.1, node.radius + 4) : node.radius + 4;
      const dx = node.x - wx,
        dy = node.y - wy;
  ...
  const draw = useCallback(() => {
    ...
    for (const edge of edges) {
      ...
      const edgeToIgnoredTag =
        (a.type === 'tag' && ignoredTagsRef.current.has(a.label)) ||
        (b.type === 'tag' && ignoredTagsRef.current.has(b.label));
      ...
      const isTrackedTagEdge =
        trackedTagNodeIds.has(edge.source) || trackedTagNodeIds.has(edge.target);
      ctx.strokeStyle = edgeToIgnoredTag
        ? 'rgba(110, 118, 129, 0.25)'
        : isHoveredTagEdge || isTrackedTagEdge
          ? 'rgba(35, 134, 54, 0.85)'
          : isActive
            ? 'rgba(88, 166, 255, 0.7)'
            : edgeColor(edge.type);
      ctx.lineWidth = edgeToIgnoredTag
        ? 1
        : Math.min(1 + edge.weight * 0.6, 4) * (isActive ? 1.5 : 1);
      ctx.stroke();
      ctx.setLineDash([]);
    ...
    for (const node of nodes) {
      ...
      if (node.type === 'stash')
        drawStashNode(ctx, node, !!isHovered, !!isConnected, dimmed, isAnalysed);
      else if (node.type === 'tag')
        drawTagNode(ctx, node, !!isHovered, !!isConnected, dimmed, isIgnoredTag);
      else drawVersionNode(ctx, node, !!isHovered, !!isConnected, dimmed);

      // Label
      const fontSize =
        node.type === 'stash'
          ? Math.max(16, Math.min(20, node.radius * 1.05))
          : Math.max(14, Math.min(18, node.radius * 1.15));
      ...
      if (zoom < 0.35) {
        ...
        if (isHovered || isAnalysed) {
          ctx.fillStyle = labelColor || (isAnalysed ? '#a5d6ff' : '#e6edf3');
        ...
        if (isHovered || isAnalysed || (node.type === 'stash' && !dimmed) || isIgnoredTag) {
          ctx.fillStyle =
            labelColor ||
            (isAnalysed ? '#a5d6ff' : isHovered ? '#e6edf3' : 'rgba(230, 237, 243, 0.5)');
          const label =
            node.type === 'stash' && node.label.length > 20
        ...
        ctx.fillStyle =
          labelColor ||
          (isAnalysed ? '#a5d6ff' : isHovered ? '#e6edf3' : 'rgba(230, 237, 243, 0.7)');
        const label =
          node.type === 'stash' && node.label.length > 20
            ? node.label.slice(0, 18) + '…'
            : node.label;
  ...
  // Re-include every ignored tag. Tags can be ignored from any stash popup,
  // but until now the only way back was "Reset analysis" — which is hidden
  // unless an analysis is active and also wipes the analysis itself.
  const handleClearIgnoredTags = () => {
    const empty = new Set<string>();
    ignoredTagsRef.current = empty;
    setIgnoredTags(empty);
  };
    ...
    <>
      <div className="graph-actions" style={{ marginBottom: 12 }}>
        ...
        {ignoredTags.size > 0 && (
          <button
            type="button"
            className="graph-ignored-tags-info graph-ignored-tags-clear"
            onClick={handleClearIgnoredTags}
          ...
          >
            ...
            <span className="graph-ignored-tags-clear-hint">clear</span>
          </button>
        )}
        {hoveredLabel && !popup && (
          ...
          >
            ...
            <div className="stash-graph-popup-meta">
              {popup.node.file_count !== undefined && (
                <span>
                  {popup.node.file_count} {popup.node.file_count === 1 ? 'file' : 'files'}
                </span>
              )}
            ...
            {popup.node.tags && popup.node.tags.length > 0 && (
              <div className="graph-popup-section">
                <div className="graph-popup-section-title">
                  Tags <span style={{ fontSize: '0.75em', opacity: 0.5 }}>(click to ignore)</span>
                </div>
        ...
        {popup && popup.node.type === 'tag' && (
          ...
          >
            ...
            {popup.node.count !== undefined && (
              <div className="stash-graph-popup-meta">
                <span>
                  {popup.node.count} Stash{popup.node.count !== 1 ? 'es' : ''}
                </span>
              </div>
            )}
            ...
            <div className="graph-popup-actions">
              <button
                ...
                onClick={() => {
                  handleToggleIgnoreTag(popup.node.label);
                  setPopup(null);
                }}
              >
                <svg
              ...
              <button
                ...
                style={
                  trackedTags.has(popup.node.label)
                    ? {
                ...
                onClick={() => {
                  handleToggleTrackTag(popup.node.label);
                  setPopup(null);
                }}
              >
                <svg
