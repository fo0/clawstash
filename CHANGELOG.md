...
- Settings → API & Tokens: the tab bar remembers which tab was last used (API Tokens / REST API / MCP API) instead of resetting to API Tokens on every visit — the stash viewer has remembered its own tab all along. Anyone working out of the MCP tab, the surface that hands an agent its onboarding prompt, had to re-select it each time. Only an explicit tab click is remembered, so a first-time user still lands on API Tokens
