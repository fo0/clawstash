...
**Fast mode** (`/fast`): the session's model at faster output — not a smaller model and not a downgrade, and offered
only on the model families that support it. Use it when latency beats reasoning depth; switch back for anything where
...
nowhere, `/orca status` reports state without changing it. Anything else after the name is an **objective run** —
`/orca <objective>` at the current width, `/orca <N> <objective>` at a stated one: steps with a review each, one
overall review over the combined diff, closed through `/done`. While on, the agent itself does no task work — every
unit goes to a role-framed subagent at the session's model and effort. Full contract — what the orchestrator keeps,
quality parity, disjoint write scopes, the argument grammar, the wave report: `.claude/skills/orca/SKILL.md`.
