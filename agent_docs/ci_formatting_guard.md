...
Hand-edited files -- long Markdown tables especially (`BACKLOG.md`, the env-var table in `agent_docs/env-vars.md`) -- drift from
...
`.prettierignore` deliberately excludes `.claude` -- the optimizer writes those files from its templates, not
Prettier-formatted, which would otherwise break `format:check` in CI -- plus the runtime/data paths (`data`, `*.db`,
`*.sqlite`, `build-info.json`). Any pre-commit formatter must honor `.prettierignore`, or it will fight every optimizer
re-run and the SQLite data dir on every commit. Keep the exclusions and the guard together; they are complementary, not
alternatives.
...
`CLAUDE.md -> Git Conventions` carries the one pointer line (_Formatting guard_) -- nothing to add there, and never a
