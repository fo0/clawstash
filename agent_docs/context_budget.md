...
> `BACKLOG.md` is deliberately **not** budgeted -- it is read on demand (only when the user asks "what's open"), so its size costs nothing per session.
...
| --- | ------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| 1   | API/endpoint/event/protocol tables beyond a one-line style summary  | `docs/api-reference.md` (this project keeps user-facing API docs there)                                                                                                                                                                                                                                                                                                                                                                                                    | `Full API reference: docs/api-reference.md`                                            |
...
| 4   | Deployment detail beyond trigger + pipeline path + agent scope      | `docs/deployment.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `Deployment detail: docs/deployment.md`                                                |
| 5   | Project structure below the second directory level                  | `agent_docs/project-structure.md`                                                                                                                                                                                                                                                                                                                                                                                                                                          | `Full tree: agent_docs/project-structure.md`                                           |
...
| 8   | Development Notes: setup hints, quirks, history                     | `agent_docs/development-notes.md`, `MEMORY.md` (stable) or `SCRATCHPAD.md` (temporary)                                                                                                                                                                                                                                                                                                                                                                                     | drop the section if it empties                                                         |
| 9   | Coding conventions beyond ~8 bullets                                | repo-wide ones -> `agent_docs/coding-conventions.md`. Conventions that hold for **one subtree only** -> `.claude/rules/{topic}.md` with `paths:` frontmatter naming that subtree's globs: Claude Code loads such a rule only while a matching file is being read, so it costs no context anywhere else and does not depend on an agent remembering to open a doc. A rule file **without** `paths:` loads every session like CLAUDE.md itself -- never write one to offload | `Full conventions: agent_docs/coding-conventions.md` · `Subtree rules: .claude/rules/` |
...
**Feature documentation is not an offload candidate -- it is a mistake.** CLAUDE.md says _how to work here_, not _what each feature does_. If a section documents behavior, delete it and let the agent read the code (or point at `README.md`, which already carries the user-facing feature list).
