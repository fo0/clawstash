...
8. **IDs are globally unique and never reused** — the next `#` is `max(id) + 1` across **both** `## Open` and `## Done`, not `max(id)` of the section you are writing into. An ID keeps its number when the entry moves from Open to Done. Reuse breaks `Closes BACKLOG #N` commit traceability: `#120`, `#121` and `#122` were each handed out twice (June vs. July 2026 review batches) because a batch restarted numbering from the Open section alone. **An entry extracted into a GitHub issue leaves both tables but keeps its number forever**, so `max(id)` over the tables alone is not the high-water mark — take the maximum over `## Open`, `## Done` **and** the retired IDs recorded in the `Extractions` block at the top of `BACKLOG.md`, then add 1. Skipping that block is how `#149` was handed out twice: retired to issue #458 on 2026-08-05, reissued on 2026-09-05. The live twin is carried in the rule-9 dated form (`#149 (2026-09-05)`); the retired one has no row left to date, so cite it by its issue. The same trap is live — `#153` went to issue #589, so the tables stop at `#152` while the next free ID is `154`.
