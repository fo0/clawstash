# Key Patterns -- clawstash

Detailed pattern descriptions for clawstash internals. CLAUDE.md keeps a short index; this file holds the depth.
...
- Size limits per field: name (500), description (50K), tags (50 x 100 chars), metadata (50 keys), files (100 x 255 char filenames, 10MB content each), language (50 chars)
...
- **`src/server/agent-guide.ts`**: Single source of truth for the operational agent guidance -- limits (imported from validation.ts / the stores, never retyped), the endpoint map, the shared prose blocks (when to store, workflow, conventions, errors, maintenance), the compact MCP `instructions`, the SKILL.md (`GET /api/agent-skill`, MCP resource `clawstash://guide/skill`) and `/llms.txt`. mcp-spec.ts builds the onboarding guide from the same blocks; `get_server_info` returns the same limits + endpoints as JSON. Also home of `memoizeByBaseUrl`. Rule: new agent-facing prose goes here once, the surfaces only frame it -- and every number in it is an import
...
- **ForceAtlas2-inspired physics**: degree-proportional gravity, 1/dist repulsion (longer-range than 1/dist^2), cross-cluster repulsion boost, weight-proportional edge attraction with log-scaled ideal distance, velocity damping + speed cap
...
- **Renders are serialized globally** (`renderChain` promise in `mermaid.ts`): Mermaid keeps mutable global state for the duration of a `render()`, so overlapping calls corrupt each other into blank/broken SVGs. At most one render runs at a time, app-wide.
- **Two render paths sharing the util:**
  - Standalone `.mmd` / `.mermaid` files -> React component `<MermaidDiagram>` rendered directly in `StashViewer` (SVG held in component state)
  - Inline ` ```mermaid ` blocks in Markdown -> marked custom `code` renderer emits `<div class="mermaid-placeholder" data-mermaid-source="BASE64">`. The rendered blob is wrapped in `<MarkdownBody html={…}>` (a `React.memo`) and `hydrateMermaidPlaceholders(root)` (`mermaid-hydrate.ts`) is called from a `useEffect` in `StashViewer` to fill each placeholder. The surrounding HTML stays a single `dangerouslySetInnerHTML` blob so nested `<table>` side-by-side layouts survive.
- **The `MarkdownBody` memo is load-bearing for F5 (#286).** StashViewer re-renders many times during a full page load; React re-applies the blob's `dangerouslySetInnerHTML` on each re-render, tearing down + recreating the `.mermaid-placeholder` nodes (and any hydrated SVG). The final re-apply landed after the hydration effect, so diagrams were blank on F5 / direct URL (fine via SPA nav, which is idle). Memoising the blob on its (stable) HTML string applies it once so the placeholders survive. **Do not remove the memo.** Verified with a real headless-browser repro and guarded by `MarkdownBody.test.tsx`.
- **Inline hydration is also orphan-proof (defence in depth).** `hydrateMermaidPlaceholders` claims each placeholder **synchronously** (`data-mermaid-rendered`, so re-entrant calls skip in-flight nodes) and writes the SVG when the lazy render resolves, guarded **only by `document.contains(el)`** — never by a React-effect `cancelled` flag. **Do not** render inline blocks via React components portaled into the placeholder nodes — React re-creates a `dangerouslySetInnerHTML` subtree on every parent re-render, which detaches the portal (verified; tried and reverted).
...
- Factory function `createMcpServer(db, baseUrl, auth)` in `mcp-server.ts` registers all tools. `auth` (`McpAuthContext`) is REQUIRED and carries the caller's scopes -- the route passes the scopes it resolved
- **The stdio entrypoint has its own door:** `createLocalStdioMcpServer(db)` (no `auth`, no `baseUrl`) is the only way to a full-trust server. `LOCAL_STDIO_AUTH` (full local trust, no token to check) is module-private behind it, so a request-served module cannot import full trust and pass it to `createMcpServer` -- it can only pass the scopes its own caller holds
- Tool definitions (name, scope, description, Zod schema) imported from `tool-defs.ts` -- only handlers are defined in mcp-server.ts
- **Scope gating lives in the local `register()` helper**, the only path to `server.registerTool` -- a new tool cannot bypass it. Required scope comes from `def.scope` in tool-defs.ts, checked with `hasScope()` from `auth.ts` (same ladder as REST: `admin` implies all, `write` implies `read`). `mcp` is a transport gate, never a capability; a denied call returns a normal tool error (`isError: true`), never an exception. "Only path" is enforced, not just asserted: `mcp-server-scopes.test.ts` fails if `.registerTool(` appears more than once in the file, if `LOCAL_STDIO_AUTH` is exported again, or if any module under `src/app/**` mentions it
