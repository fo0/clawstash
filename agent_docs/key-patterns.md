...
## Mermaid Rendering (src/utils/mermaid.ts, src/utils/mermaid-hydrate.ts, src/components/MermaidDiagram.tsx)
...
- **Two render paths sharing the util:**
  ...
  - Inline ` ```mermaid ` blocks in Markdown -> marked custom `code` renderer emits `<div class="mermaid-placeholder" data-mermaid-source="BASE64">`, then `hydrateMermaidPlaceholders(root)` (`mermaid-hydrate.ts`) is called from a `useEffect` in `StashViewer` to fill each placeholder. The surrounding HTML stays a single `dangerouslySetInnerHTML` blob so nested `<table>` side-by-side layouts survive.
- **Inline hydration is orphan-proof (#286).** `hydrateMermaidPlaceholders` claims each placeholder **synchronously** (`data-mermaid-rendered`, so re-entrant calls skip in-flight nodes) and writes the SVG when the lazy render resolves, guarded **only by `document.contains(el)`** — never by a React-effect `cancelled` flag. The original bug: the write was gated by effect cancellation, so a re-render during page boot cancelled the in-flight render and the write never landed (blank on full page load / F5, fine via SPA nav). **Do not** reintroduce a cancellation-gated write, and **do not** try to render inline blocks via React components portaled into the placeholder nodes — React re-creates a `dangerouslySetInnerHTML` subtree on every parent re-render, which detaches the portal (verified; that approach was tried and reverted).
