...
Full environment-variable reference. CLAUDE.md carries only the 3 variables an agent must know; everything else lives here. Placeholders + comments: `.env.example`.
...
> Both boolean-ish and numeric variables fail silently rather than loudly: `TRUST_PROXY` is an exact string compare against `'1'` / `'true'` (`src/server/auth-rate-limit.ts` -> `isTrustedProxy`, inlined again in `src/middleware.ts` -> `isHttpsRequest` because the Edge runtime cannot import that Node-only module -- change both together), so `yes` / `on` / `TRUE` leave forwarded headers untrusted; `ADMIN_SESSION_HOURS` (`src/server/auth.ts`) rejects NaN, negative and infinite values back to `24`. Verify the effective behaviour after setting them -- a typo looks like the default, not like an error. `ADMIN_PASSWORD` behaves the same way in the other direction: an empty value (`ADMIN_PASSWORD=`) is indistinguishable from unset (`process.env.ADMIN_PASSWORD || ''` in `src/server/auth.ts`) and leaves the instance in open mode, which `src/instrumentation.ts` announces with a `[security] ... OPEN MODE` warning at startup. `CLAWSTASH_ENCRYPTION_KEY` is the one validated variable that fails **loudly**: anything that is not exactly 64 hex characters throws on first use (`src/server/backup/backup-crypto.ts`) rather than falling back to the auto-generated key file, and a corrupt key file throws too instead of being regenerated.

> Two silent edge cases the note above flattens. `ADMIN_SESSION_HOURS` goes through `Number.parseFloat` (`src/server/auth.ts` -> `ADMIN_SESSION_HOURS`), which reads only the leading number: `8h` becomes `8`, and `00:30`, `0x10`, `0h` or `-0` become `0` -- a session that never expires, not the `24` fallback. Only a value without a leading number, a negative number or an infinite one falls back to `24`. An **empty** `CLAWSTASH_ENCRYPTION_KEY` (`CLAWSTASH_ENCRYPTION_KEY=`) is not validated at all: `getEncryptionKey()` (`src/server/backup/backup-crypto.ts`) treats it as unset and reads or generates the key file `dirname(DATABASE_PATH)/.clawstash-key` without an error or a log line -- only a non-empty value that is not 64 hex characters throws. The key-file suite in `src/server/backup/__tests__/backup-crypto.test.ts` stubs the variable to `''` and pins that path.
...
| At-rest encryption key | `CLAWSTASH_ENCRYPTION_KEY`, or the auto-generated key file `dirname(DATABASE_PATH)/.clawstash-key` (default `data/.clawstash-key`, mode 0600). The default lands in the gitignored `data/`, but the path follows `DATABASE_PATH` — point that at a directory inside the repo and the key file is no longer covered by an ignore rule | Yes          |
...
- The at-rest key is the only secret that lives **inside the data volume** rather than in the environment, and the only one whose loss is unrecoverable — a volume backup carries it, a database-file-only backup does not. Restoring a database without the matching key means the stored GitHub-backup token cannot be decrypted and the connection has to be re-established (no stash data is lost). `src/server/backup/backup-crypto.ts`, `docs/backup.md`, `docs/deployment.md`.
...
- The `basic-sec-review` skill scans for committed secrets.
