...
Full convention list. CLAUDE.md keeps the few an agent violates most easily; the rest -- component style, CSS conventions, error handling and the TS compiler settings -- is here.

- **Language**: All UI text and documentation in English.
- **Module System**: ESM (`"type": "module"` in `package.json`).
- **Formatting**: 2-space indentation, single quotes in TS. `.claude/` is excluded via `.prettierignore` (tool-managed files; the optimizer writes them from its templates, not Prettier-formatted) -- keep that exclusion.
...
Lives in `agent_docs/autonomy.md -> Never-translate term list` (canonical, next to the other _Output Languages_ elaborations) -- not restated here.
