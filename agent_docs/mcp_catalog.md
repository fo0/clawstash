...
Merge additively into an existing file; never remove user entries. To re-gate a single tool (e.g. `add_repo`), add it to `permissions.ask` -- `ask` is evaluated before `allow`, so it prompts despite the glob. **The agent never writes this file on its own** -- it lives outside the repo, so applying it is the user's call.
