...
├── .prettierignore             # Prettier excludes (`.claude/` is excluded on purpose -- CLAUDE.md -> Coding Conventions)
...
├── AGENTS.md                   # Pointer to CLAUDE.md for agent tools that read the cross-vendor filename
...
│   ├── review_process.md       # Review process (on demand via the `basic-review` skill, never auto-run)
...
├── .claude/
│   ├── settings.json           # Tier-1 hooks (session start, context budget, quality-config guard) + trigger allowlist
│   ├── loop.md                 # Default prompt for a bare `/loop`
│   └── skills/                 # Agent workflow skills: done, pr, basic-review, basic-sec-review, rollback, ci, stuck, beacon, scheduler, orca, verify
├── .github/
