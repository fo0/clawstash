...
- Call `compat_whoami` and confirm it agrees with this repo ("fo0/clawstash", i.e.
  ...
  - `declares_project: true` — the shared master/onboarding token, bound to no project:
    then every call below MUST carry `project:"fo0/clawstash"` (`repo:` on report/observe),
...
- Call `compat_proposals` with `project:"fo0/clawstash"`: the open Dependabot PRs beacon
...
- Run `compat_check` with `project:"fo0/clawstash"` and the lock-exact stack (prefer the
...
- Re-run `compat_check` with `project:"fo0/clawstash"` and the new stack — it syncs the
...
- Verified something beacon did not know (an unknown/flagged pair now proven)? Write back
  ONE tight-range `compat_report` rule (`repo:"fo0/clawstash"`) for the pair you actually
  exercised — never blanket-report every routine bump.
- Updates you decided against — or that failed — record with `compat_decide`
  (`project:"fo0/clawstash"`, `rejected`/`failed`/`abandoned` + note), so the project
  dashboard shows why.
