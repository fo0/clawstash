...
import type { ClawStashDB, TokenScope } from './db.js';
 ...
 */
export function requireAdminAuth(db: ClawStashDB, req: Request): AuthResult | null {
  if (!isAuthEnabled()) {
 ...
 */
export function requireScopeAuth(db: ClawStashDB, req: Request, scope: TokenScope): AuthResult | null {
  if (!isAuthEnabled()) {
    return { authenticated: true, source: 'open', scopes: ['read', 'write', 'admin', 'mcp'] };
  }
